...
public class PlayerMovement : MonoBehaviour
{
	...
	public void setHorizontalMove(float hm) {
		horizontalMove = hm;			
	}
    ...
    void Update() {
        ...
        if (Input.GetButtonDown("Dash") && !jump)
        {
            Debug.Log("asdf");
        ...
        // 대시키가 눌린경우
        if(dash && !jump && horizontalMove != 0f)
        {
    ...
    public void OnCrouching(bool inCrouching) {
        FindObjectOfType<AudioManager>().Play("PlayerCrouching");
    ...
    private void OnCollisionEnter2D(Collision2D collision) {
        ...
        {
            // 점프로 밟아 충돌한경우
            if (jump && collision.gameObject.transform.position.y <= gameObject.transform.position.y)
            {
